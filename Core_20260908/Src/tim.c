...
void MX_TIM3_Init(void)
{
  ...
  /*
   * 四路都要开输出比较预装载, 不能只开 CH4。
   *
   * 不开预装载时写 CCR 是立即生效的: PWM 模式 1 下输出在 CNT < CCR 期间为高,
   * 若在 CNT 已越过旧 CCR (输出已拉低) 之后把 CCR 调大, 输出会在同一个 20ms
   * 帧里第二次拉高 —— 舵机收到一帧双脉冲, 于是抽一下。
   * 开了预装载后新值只在下一次更新事件 (帧边界) 才载入影子寄存器, 每一帧要么
   * 全是旧脉宽要么全是新脉宽, 不会出现半帧畸形。
   *
   * 撞上的概率 = 脉宽变化量 / 20000: 后斗 0→110° 是 500→1722us, 约 6%,
   * 表现为"偶尔在下指令的瞬间抽一下", 不容易复现, 所以以前只在滚刷上发现过。
   */
  ...
  __HAL_AFIO_REMAP_TIM3_DISABLE(); /* CH4 固定 PB1 */
...
void HAL_TIM_Encoder_MspInit(TIM_HandleTypeDef* tim_encoderHandle)
{
  ...
  /* USER CODE BEGIN TIM2_MspInit 1 */
    /* MAPR 改 remap 会把 SWJ 写乱，必须再打开 SWD，否则下次下载 timeout */
