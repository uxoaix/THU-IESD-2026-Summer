...
/* 仅为 HOME_DONE 的卸货闸门取 HAL_GetTick(): 闸门要的是"距上电多久",
 * 用状态机自己累加的计时不行 —— MotionStrategy_Init() 每次进 AUTO 都会清零,
 * 中途手动切一次就把闸门重新起算了。 */
...
/* BLACK_AREA_TRACK 里"看得见黑区"的累计时长, 卡死兜底用。只在检出的周期累加,
 * 看不见时暂停而不清零 —— 判的是"追了这么久还没追成", 断续丢帧不该让它归零。 */
static uint32_t s_black_track_ms;
                           ...
                           MotionCommand_t *out)
{
  ...
  case MOTION_STATE_BLACK_AREA_TRACK:
     ...
     * 卡死兜底：两个判据凑不齐 BLACK_TRACK_GIVEUP_MS 就倒一段再来，见配置说明。
     */
    if (s_action_step == 1U) {
      /* 倒车中。退完清掉三个计时，回追踪重新给一个完整的 GIVEUP 窗口。 */
      if (s_state_ms >= BLACK_TRACK_BACKUP_MS) {
        Motion_StopOutput(out);
        s_arrive_confirm_ms = 0U;
        s_align_confirm_ms = 0U;
        s_black_track_ms = 0U;
        ...
        Motion_Line(-WALL_BACKOFF_SPEED_CM_S, out);
      }
      ...
      /* 卡死计时只在看得见黑区的周期走，没检出就不算数。 */
      s_black_track_ms += elapsed_ms;
      ...
      if (s_arrive_confirm_ms >= BLACK_ARRIVE_CONFIRM_MS &&
          s_align_confirm_ms >= BLACK_TURN_ALIGN_CONFIRM_MS) {
        BeginRotateCw(heading, HOME_TURN_AROUND_TARGET_DEG);
        Enter(MOTION_STATE_HOME_TURN_AROUND);
      } else if (s_black_track_ms >= BLACK_TRACK_GIVEUP_MS) {
        /* 排在掉头之后：这一周期两个判据都齐了就该掉头，不该反而退回去。 */
        Motion_StopOutput(out);
        s_action_step = 1U;
        s_state_ms = 0U;
      } else if (s_arrive_confirm_ms >= BLACK_ARRIVE_CONFIRM_MS) {
  ...
  case MOTION_STATE_HOME_DONE:
     ...
     *
     * 倒车前先过卸货闸门：上电计时没到 UNLOAD_GATE_FROM_BOOT_MS 就原地停着等，
     * 车此刻已经掉头对好黑区，等到点再退进去。见配置里的副作用说明。
     */
    if (HAL_GetTick() < UNLOAD_GATE_FROM_BOOT_MS) {
      Motion_StopOutput(out);
      /* 等待期间不让状态计时累积，否则放行那一刻已经超过倒车时长，
       * 会被判成"倒完了"直接进卸货，一步都不退。 */
      s_state_ms = 0U;
      break;
    }
  ...
  case MOTION_STATE_BLACK_AREA_SEARCH:
    ...
    if (s_seen_ms >= SEARCH_DETECT_CONFIRM_MS) {
      /*
       * 与 ROTATE_SEARCH 同一道去抖：黑区门限只有 300px(一帧的 0.4%)，扫视时
       * 画面在动，阴影和深色物体都够触发单帧误检；而一旦进了追踪，要丢满
       * TARGET_LOCK_WINDOW_MS 才退得回来，误检的代价是朝虚假目标开近 1s，
       * 回来时扫视基准和方向表还被重置一遍。
       * 攒满之后跳过纯原地对准直接追踪：黑区面积大，边走边对就够，这一条与
       * 去抖是两件事，加了去抖也不用改。
       */
      Enter(MOTION_STATE_BLACK_AREA_TRACK);
    } else if (BackoffIfWallSeen(vwall, out)) {
