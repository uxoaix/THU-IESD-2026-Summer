...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
  ...
  {
    OpenMvUart_OnRxComplete();
  }
  else if (huart->Instance == USART3)
  {
    /* JY901S IMU 在 USART3@115200, 收姿态数据 */
    IMU_OnUartRxComplete();
  }
  else if (huart->Instance == UART4)
  {
    /* HC-05 蓝牙在 UART4@115200, 收电脑端 STOP/RESUME 指令 */
    Hc05Uart_OnRxComplete();
  }
}
...
void HAL_UART_ErrorCallback(UART_HandleTypeDef *huart)
{
  ...
  {
    /* IMU 溢出等错误后重启接收, 保证数据流不断 */
    IMU_OnUartError();
  }
  else if (huart->Instance == UART4)
  {
    /* HC-05 溢出等错误后重启接收, 保证数据流不断 */
    Hc05Uart_OnError();
  }
}
