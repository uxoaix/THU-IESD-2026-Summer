 ...
 * 两条航向都要给: 融合航向定几何 (与 x/y 同源), 纯 IMU 航向供之后的角度闭环。
 */
void ImuOdometry_BeginReturn(void)
{
  HomeTrajectory_BeginReturn(SensorFusion_GetHeading(),
                             SensorFusion_GetImuHeading());
}
...
uint8_t ImuOdometry_GetReturnCommand(float heading_rad,
                                     ...
                                     float *angular_rad_s)
{
  return HomeTrajectory_GetReturnCommand(heading_rad,
                                         SensorFusion_GetImuHeading(),
...
/*
 * 纯 IMU 航向 (0~359), 已过 IMU_YAW_SCALE。HOME_FOLLOW 锁定后的保向闭环走的
 * 就是这一路, 所以这个字段现在也是"控制实际看到的角度"。
 * 与 yaw_enc / hdg_deg 同零点, 三者对照可定位标度误差归属: 原地转过已知角度
 * 后, 谁的增量偏离实际转角, 问题就在谁身上 —— 注意本值已乘 0.83, 要看模组
 * 的原始输出得先除回去。
 */
