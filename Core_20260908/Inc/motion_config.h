...
/* 上电控制模式: 0=一直手动等蓝牙 AUTO, 1=上电 AUTO_START_DELAY_MS 后自启动。
 * 无论哪个, 起步瞬间都是手动态; 运行中可用 MANUAL/AUTO 随时切换。 */
#ifndef DEFAULT_CONTROL_MODE
#define DEFAULT_CONTROL_MODE                0U  /* 1=上电延时自启动 */
#endif
...
/*
 * 扫视阶段发现目标的去抖: 要求 wanted_detected 连续为真这么久才离开扫视,
 * 不再一帧就跳 (约 6 个 MOTION_PERIOD_MS 周期)。两处共用:
 *   ROTATE_SEARCH      找红/黄物块 → PRE_CENTERING
 *   BLACK_AREA_SEARCH  找卸货黑区  → BLACK_AREA_TRACK (黑区面积大, 不走对准)
 *
 ...
 * 代价: 目标出现到开始处理多 300ms, 期间扫视又转过一点 (物块 35°/s 约 10°,
 * 黑区 50°/s 约 15°), 目标会更偏离画面中心, 这个偏差由后面的 PRE_CENTERING /
 * BLACK_AREA_TRACK 自己拧回来。太大 (比如 1s) 会让快速掠过视野的目标错过。
 */
#define SEARCH_DETECT_CONFIRM_MS         300U
/*
 * 单轮原地扫视的时间兜底, 与 SEARCH_FULL_CIRCLE_DEG 并列 (谁先到谁生效)。
 * 它只该在 IMU 异常、航向不涨时救场, 正常情况要让角度判据先到, 所以必须留足
 * 转满一圈的时间: 400° / 35°/s = 11.4s 是理论值, 滑移转向实际转速低于指令值,
 * 再留两成余量给到 14s。改扫视转速或整圈角度时这里要跟着改。
 */
#define SEARCH_NO_TARGET_TIMEOUT_MS       14000U  /* 10000 */
/*
...
#define TRACKING_LINEAR_SPEED_CM_S       40.0f     /* 20.0 */
...
/*
 * BLACK_AREA_TRACK 卡死的兜底: 累计看见黑区这么久, 但"到位"和"对准"始终凑不齐,
 * 就倒一段再重来。
 *
 * 计时只在检出黑区的周期累加, 没检出就暂停 (不清零): 判的是"追了这么久还没
 * 追成", 所以没看见的时间不该算进来; 而断续丢一两帧就归零的话, 门限永远攒不满,
 * 兜底等于没有。真丢了目标由 TARGET_LOCK_WINDOW_MS 那条路回搜索, 不归本计时管。
 *
 * 为什么需要: 本状态原来没有任何超时 —— 目标不丢就不会走 ResumeSearch, 又不
 * 做蓝墙退避 (黑区贴着墙, 退避会让它永远卸不了货), 于是能无限期停在这里。
 * 典型卡法是车头顶住黑区边缘的挡板: 推不动, 面积比又差一点到不了阈值。
 *
 * 为什么倒车能解: 压得太近时黑区糊满画面, 外框被裁切, 占比和中心都失真; 退开
 * 让整块黑区重新落进视野, 两个判据才算得准。同时也把顶住的地方脱开。
 *
 * 退完回到追踪并重新计时, 不设放弃次数: 这一步只是打破僵局, 真丢了目标自有
 * TARGET_LOCK_WINDOW_MS 那条路回搜索。1.0s × 17.25 ≈ 17cm, 退一个车身左右。
 */
#define BLACK_TRACK_GIVEUP_MS            10000U
#define BLACK_TRACK_BACKUP_MS            1000U
...
#define TOTAL_OBJECTS_TO_COLLECT         6U  /* 5 */
 ...
 *       走过 HOME_VISION_ENABLE_RATIO (5%) 之后就开始认黑区, 认到随时直接
 *       交给 BLACK_AREA_TRACK, 不必等直线段走完。
 *
...
/*
 * 卸货区相对上电位置的横向偏移: 正=在上电朝向的右侧。
 *
 * 开机时车停在起点, 但要返回的黑区并不在脚下, 而在右侧这么远。所以建原点时
 * 不能把车当成 (0,0), 而要把黑区当成 (0,0) —— 车的初始坐标记成 (0, +本值)。
 * 坐标系里 +x 是上电朝向、+y 是它的左侧, 原点在车右侧就等于车在原点左侧。
 * 这样第一趟返航的方位角 atan2(-y,-x) 直接指向右侧, 车会先右转再开过去。
 *
 * 只影响 ImuOdometry_SetHome() (开机建系与蓝牙 SET_HOME)。第一次卸货后改用
 * MoveOriginHere(), 偏移传 0 —— 那时车就在真实卸货点上, 不需要再猜。
 * 填 0 = 沿用"上电位置就是家"的老行为; 填负值 = 黑区在左侧。
 * 遥测上电时会显示 home_y=60 而不是 0, 这是正常的。
 */
#define HOME_START_OFFSET_RIGHT_CM       60.0f
/* 直线段只走全程的这个比例, 余下距离作为退出阈值。
 * 调大 = 更依赖里程计精度, 调小 = 更早交给视觉但可能离黑区太远看不见。 */
#define HOME_PARTIAL_RETURN_RATIO        0.5f  /* 0.75 / 0.2 */
/*
 * 走过全程的这个比例之后才解锁视觉交接; 在此之前是纯里程计返航 —— 朝锁定的
 * 方位角开, 看见黑区也不理。
 *
 * 这道门原本是为了压后交接: 场地上有两个黑色卸货区 (自己的和对方的),
 * OpenMV 只认"黑色色块"、分不出是谁的, 返航刚起步时车在场地远端, 最容易先
 * 看见对方那个并一头扎过去。所以曾取 0.5 (走过一半才认), 后来 1/3。
 *
 * 现取 0.05, 等于起步走一小段就开始认 —— 权衡换了方向: 宁可尽早用视觉锁住
 * 黑区, 也不要因为门开得晚而错过自己家、最后靠原地扫视慢慢找。误认对方卸货
 * 区的风险因此回升, 若实测出现扎向对方黑区的情况, 往回调到 0.3~0.5。
 * 保留一点点而不是直接填 0 的理由: 起步那一两帧车还没离开上一轮的卸货点,
 * 脚下就是黑区, 会立刻自我触发一次交接。
 *
 * 注意这道门只拦"认不认", 不切模式: OpenMV 返航全程都留在黑区模式 (M,1)。
 * 解锁前改回物块模式没有意义 (返航途中不收物块), 还要在解锁点再切回来、
 * 白等 OpenMV 重新稳定几帧, 而黑区到位帧 (A) 也只在黑区模式下发。
 *
 * 视觉窗口 = 本值到 HOME_PARTIAL_RETURN_RATIO 这一段, 现在是 5%~50%。
 * 窗口内没确认到就走到直线段终点, 转 BLACK_AREA_SEARCH 原地扫, 仍能找到,
 * 只是慢一些。
 *
 * 注意必须小于 HOME_PARTIAL_RETURN_RATIO, 否则视觉窗口为空, 等于关掉了
 * 途中交接。
 */
#define HOME_VISION_ENABLE_RATIO         0.05f  /* 1/3 / 0.5 */
...
/*
 * 卸货闸门: 上电计时不到这么久, HOME_DONE 就原地停着不倒车。
 *
 * 用途是把卸货压到比赛末段: 车可以提前收满并掉头对好黑区, 但要等到点才退进去
 * 倒货。取 HAL_GetTick() 即真正的上电时间, 不受 MANUAL/AUTO 来回切换影响。
 *
 * 【副作用要清楚】闸门是拦在 HOME_DONE 这一个点上的, 车在那儿就是干等着,
 * 不会回去继续找物块。所以只要提前返航就会白等: 比如 SEARCH_GIVE_UP_MS 到点
 * (距上次收集 30s 没收到) 会在任意时刻触发返航, 若那时才 1 分钟, 车就在黑区
 * 前面停三分半。要避免这种浪费, 得让闸门期间回去继续搜索, 那是另一套逻辑。
 * 闸门期间每周期都把 s_state_ms 清零, 放行后才完整走一遍倒车时长。
 */
#define UNLOAD_GATE_FROM_BOOT_MS         270000U  /* 4 分 30 秒 */
 ...
 *
 * 【必须和 SEARCH_NO_TARGET_TIMEOUT_MS 一起看】那个时间兜底是并列的 or 判据,
 * 谁先到谁生效。本值 / SEARCH_SWEEP_ANGULAR_DEG_S 就是转满所需的时间, 必须
 * 明显小于那个超时, 否则超时先到, 本值等于没改:
 *   400 / 35 = 11.4s, 而超时给的是 14s (含打滑余量)。
 * 改本值或改扫视转速时, 都要回头核一遍这个不等式。
 */
#define SEARCH_FULL_CIRCLE_DEG           400.0f /* 350.0 */
 ...
 *         → 左前弧线开出 → 关门。
 *   "振颤"是抬起状态下 0↔110 全行程的快速来回, 见下面的 BUCKET_SHAKE_*;
 *   全程只升一次、只落一次, 两轮振颤都在抬起状态下完成; 每轮之前那 1s 是等斗
 *   真正抬到位, 否则振颤起点会逐轮偏低。
 *   两轮振颤之间挪一小段, 让第二轮卸在稍微不同的位置, 免得所有物块堆在
 *   同一点上互相挡住出口。
 *   门只开关一次, 从开门一直开到驶出黑区之后: 物块常卡在斗底或门边, 只有门
 *   开着时反复升降才能把它抖出去; 每轮都开关门既慢, 抖动也只在开门那一刻才
 *   有效。关门放在开出黑区之后, 是为了让开出那 2.8s 的颠簸也能继续抖料。
 *   升斗/落斗要等舵机真的走完行程; 振颤反过来, 故意不等它走完。
 *   合计约 0.5 + 1 + 0.6 + 0.5 + 1 + 0.6 + 1 + 2.5 + 0.5 = 8.2s
 *   (每轮振颤 0.6 = 4 × 0.15)。 */
 ...
 *
 * 关键在于 STEP_MS 远小于走完全行程所需的时间 (整程约 BUCKET_LIFT_DURATION_MS
 * 那么久): 舵机还没到位就被下一个反向指令打断, 斗在中段附近大幅来回颤。
 * 震荡效果靠的就是这个"够不着"—— 别把 STEP_MS 加大到足以走完全程, 那样会退化
 * 成一串慢速的完整落升, 冲击反而变小、耗时成倍增加。
 * 代价: 舵机全程处于接近堵转的状态, 电流是整个流程里最大的一段。
 *
 * MOVES 必须取偶数: 振颤从抬起的 110°出发, 偶数次行程正好停回 BUCKET_END_DEG
 * 那一侧, 之后才由落斗步正常落下; 取奇数会停在 0°, 后面的落斗就成了空动作。
 * 4 次即 0→110→0→110。
 *
 * 每轮振颤之前都要先用 BUCKET_LIFT_DURATION_MS 等斗真正抬到 110°: 上一轮颤完
 * 时最后一条指令虽然是 110°, 但舵机没走完就被打断过, 实际停在中段, 不等它到位
 * 就开始下一轮, 振颤的起点会越来越低、幅度逐轮缩水。
 */
#define BUCKET_SHAKE_MOVES               4U
#define BUCKET_SHAKE_STEP_MS             150U
 ...
 * 不会连带把下一轮的家往前推。
 */
#define UNLOAD_MID_FORWARD_MS            700U
 ...
 *     · 返航链路: HOME_TURN_AROUND 掉头, 以及 HOME_FOLLOW 锁定那一刻的几何折算
 *       (只此一次; 锁定之后的保向闭环已改吃纯 IMU 航向, 见 IMU_YAW_SCALE)
 ...
 *   返航直线段锁定方位角时, 方位角与融合航向同源, 相减后本系数的影响抵消,
 *   所以"还要转多少度"这个量是干净的; 但真正把这些度数转出去的是纯 IMU 那一路,
 *   转角精度归 IMU_YAW_SCALE 管。本系数主要还是决定定角旋转的精度。 */
#define FUSION_YAW_SCALE                 0.85f
/* 纯 IMU 航向 (SensorFusion_GetImuHeading) 的标度校准。
 *
 *   >>> HOME_FOLLOW 锁定后的保向闭环现在吃这一路, 不再只是遥测 <<<
 *   闭环追的是"锁定那一刻的 IMU 读数 + 还要转多少度", 每周期只做
 *   WrapPi(target - imu_now), 所以本系数直接决定这段保向转多少。
 *   定角旋转 (ROTATE180/360、HOME_TURN_AROUND) 仍走 FUSION_YAW_SCALE 那一路。
 *
 *   0.83 的由来: 下面三行实测记录拟合出 k≈0.83, 即模组把转角多报约 20%。
 *   不校准 (1.0) 的话闭环在 err=0 时收敛, 需要转 90° 时实际只转到约 75°。
 *
 *   这个档位对 HOME_FOLLOW 恰好合适: 标定是在 BT_ROTATE_SPEED_DEG_S=45 做的,
 *   而保向的原地转饱和在 HOME_RETURN_MAX_ANGULAR_RAD_S=0.8 (≈46°/s), 两者
 *   几乎同速。之前这一路失败是因为拿 45°/s 的系数去套 ROTATE_SEARCH 的扫视
 *   转速, 档位对不上; 现在只用在保向, 不存在这个问题。
 *
 *   代价: 遥测 imu_yaw 也过这个系数, 不再是模组的原始输出, 拿它和 yaw_enc /
 *   hdg_deg 对照定位标度问题时要记得先除回 0.83。要恢复这项诊断能力就得给
 *   闭环单开一个系数, 与本值解耦。
 *
 ...
 *   标定; 光调系数追不上。 */
#define IMU_YAW_SCALE                    0.83f  /* 1.0 */
/* 卡尔曼噪声参数 (方差, 越大越不信任该源) */
